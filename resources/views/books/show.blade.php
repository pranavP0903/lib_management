...
@section('header-buttons')
    <div class="btn-group">
        <a href="{{ route('books.edit', $book->book_id) }}" class="btn btn-outline-primary">
            <i class="bi bi-pencil me-1"></i> Edit
        </a>
        <a href="{{ route('circulation.issue') }}?book_id={{ $book->book_id }}" class="btn btn-primary">
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><span class="badge bg-secondary">#{{ $copy->copy_id }}</span></td>
                                ...
                                <td>
                                    @if($copy->status == 'ISSUED' && $copy->currentBorrowing)
                                        {{ $copy->currentBorrowing->member->full_name }}
                                        <br>
                                        <small class="text-muted">Due: {{ $copy->currentBorrowing->due_date->format('M d') }}</small>
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        ...
                                        <a href="{{ route('circulation.issue') }}?copy_id={{ $copy->copy_id }}" 
                                        ...
                                        <form action="{{ route('circulation.return.submit') }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="transaction_id" value="{{ $copy->currentBorrowing->transaction_id }}">
                                        ...
                                        <button type="button" class="btn btn-outline-secondary" 
                                                data-bs-toggle="modal" data-bs-target="#editCopyModal{{ $copy->copy_id }}"
                                                title="Edit">
                                            <i class="bi bi-pencil"></i>
                                        </button>
                                        
                                        @if($copy->status == 'AVAILABLE')
                                        <form action="{{ route('books.copies.destroy', $copy->copy_id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="member_id" class="form-select" required>
                            ...
                            <option value="{{ $member->member_id }}">{{ $member->full_name }} ({{ $member->member_type }})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
<div class="modal fade" id="addCopyModal">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('books.copies.store', $book->book_id) }}" method="POST">
                @csrf
                <div class="modal-header">
