                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                ...
                                <button type="button" class="btn btn-outline-info" 
                                        data-bs-toggle="modal" data-bs-target="#borrowingDetails{{ $borrowing->transaction_id }}"
                                        title="Details">
                            ...
                            <div class="modal fade" id="borrowingDetails{{ $borrowing->transaction_id }}">
...
<!-- Bulk Fine Modal -->
<div class="modal fade" id="bulkFineModal">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('fines.bulk') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Apply Bulk Fines</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Select Overdue Borrowings</label>
                        <select name="borrowing_ids[]" class="form-select" multiple size="5">
                            @foreach($borrowings->where('is_overdue', true) as $overdue)
                            <option value="{{ $overdue->transaction_id }}">
                                #{{ $overdue->transaction_id }} - {{ $overdue->member->full_name }} ({{ $overdue->days_overdue }} days)
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="fine_per_day" class="form-control" value="5">
                    </div>
                </div>
                <div class="modal-footer">
...
function sendReminders() {
    if(confirm('Send due date reminders to all members with active borrowings?')) {
        // AJAX call to send reminders
        $.ajax({
            url: '{{ route("circulation.send-reminders") }}',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}'
            },
...
function markMultipleReturned() {
    ...
    if(confirm(`Mark ${selectedBorrowings.length} borrowings as returned?`)) {
        // AJAX call to mark as returned
        $.ajax({
            url: '{{ route("circulation.bulk-return") }}',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
